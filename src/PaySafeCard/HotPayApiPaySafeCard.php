...
class HotPayApiPaySafeCard extends WebClient
{
    ...
    )
    {
        ...
        $request = $this->doRequest('https://psc.hotpay.pl/', [
            'form_params'=>$params
        ], 'POST', false, false);

        $json = @json_decode($request->getBody());
        if(!$json) throw new PaySafeCardException('HotPay error: invalid notification password');

        if(!$json->STATUS) throw new PaySafeCardException('HotPay error: '.$json->WIADOMOSC);
